...
export function ProcessOverview() {
  return (
    <section
      id="process"
      className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-6 py-24"
    >
      ...
      <div className="grid gap-8 border border-zinc-200 md:grid-cols-4 md:divide-x md:rounded-2xl">
        {STEPS.map((step, index) => (
          <div key={step.title} className="flex flex-col gap-4 px-6 py-8">
            <div className="text-sm font-semibold uppercase tracking-[0.2em] text-[#1F3A93]">
              {index + 1 < 10 ? `${index + 1}` : index + 1}
              <span className="mx-2 text-zinc-300">&mdash;</span>
            </div>
